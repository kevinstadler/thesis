...
<<questionnairelocation, fig.width=smallwidth, fig.height=smallwidth, fig.cap="Geographical distribution of questionnaire participants in Shetland, by condition (online convenience sample vs. paper-based controlled sample)">>=
...
ormtables(nullmdl, varmdl, vargendermdl, vargenderagemdl, agegenderchangingvarmdl, fullmdl, of="of participants' own usage estimates.", label="table:selfresponsesmodelordinal")
...
groupedhistogram <- function(formula, ..., data=d, type="percent", scales=defaultscales, col=defaultbarcol, aspect=0.8, xlab="", ylab="percent of responses") { # to count or not to count?
...
groupedhistogram(~ self | longvar + agecat, main="reported own usage", col=temp.colors(5), ylim=c(0, 93))
...
groupedhistogram(~ other | longvar + gender, data=d, main="reported community usage", ylim=c(0, 90))
...
varmdl <- clme(selfdiff ~ age+var, random=id)
...
ormtables(nullmdl, agemdl, varmdl, agevarchangingmdl, agevarmdl, of="of the relative difference between participants' reported own and community-level usage.", label="table:selfdiffmodel")
...
ormtables(ovnullmdl, gendermdl, agemdl, ovstablemdl, varmdl, stableplusagemdl, stableagemdl, changingvarmdl, of="of responses to the question ``Which of the two variants do you think is \\emph{older}?''.", label="table:variantagemodel")
