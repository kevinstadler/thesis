...
groupedhistogram <- function(formula, ..., data=d, type="percent", scales=defaultscales, col=defaultbarcol, aspect=0.8, xlab="difference score", ylab="percent of responses") { # to count or not to count?
...
groupedhistogram(~ self | longvar + agecat, col=temp.colors(5), ylab="actual usage response (%)", xlab="", ylim=c(0, 93))
...
lattice::barchart(100*response ~ self | longvar + age, data=predictions, col=defaultbarcol, scales=defaultscales, ylab="predicted usage response (%)", ylim=c(0, 93), box.width=1, aspect=1, between=list(x=1, y=1))
...
groupedhistogram(~ selfdiff | longvar + agecat, data=d, col=temp.colors(9), ylim=c(0, 85), drop.unused.levels=FALSE) # , xlab="difference in number of ordinal categories between individuals' 'self' and 'other' reponses"
...
groupedhistogram( ~ oldervar | longvar + agecat, data=d, col=temp.colors(3), ylim=c(0, 90))
...
groupedhistogram(~ apparentdiff | longvar, data=d, layout=c(4,1), drop.unused.levels=FALSE, col=temp.colors(9), ylim=c(0, 48))
