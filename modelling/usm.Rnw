...
At its core, every agent in the USM is completely characterised by its variable use over variants, specified by the proportions with which each variant is used, all of which together sum to 1. For sake of simplicity we will limit ourselves to the canonical case of two competing variants, where the behaviour of an agent $i$ can be captured by a single variable~$0 < x_i < 1$ representing its relative usage level of a variant, with that of the competing variant taken to be $1-x_i$. These internal proportions drive language users' production of variants by randomly sampling from a binomial (or, in the case of more than two variants, multinomial) distribution.
...
equivalence to EWMA $\alpha$

\subsubsection{Production sample resolution $T$}

\subsubsection{Accommodation/alignment parameter $h$}

accommodation $h$: only used sparely~\citep{Baxter2016,Michaud2017}.

It is only important when there are systematic differences in usage levels within the population, as would be the case when there are clusters or differences in the degree of connectivity in a social network.

Differences could also be due to other inter-individual differences, such as age-stratified populations, or differing replicator selection biases.


For all the simulation work in this thesis agents are set to only align with their interlocutor and not to their own productions.
...
\subsubsection{Interaction frequencies $G_{ij}$}
