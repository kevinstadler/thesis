...
%TODO refer to \label{sec:realigriffithsequivalence} to point out trouble of N parameter
...
plotchains(generate.transitioning.chains(bilm.transition.matrix.sample(N, 2*alphahalved), ngenerations, nchains), main="(a)")
plotchains(generate.transitioning.chains(bilm.transition.matrix.average(N, 2*alphahalved), ngenerations, nchains), main="(b)")
@

What is also evident is that not all of the `transitions' are actually of the length that we specified: many chains either remain at the initial state for some time, or otherwise converge on categorical usage of the other variant early and remain there until the remaining generations have passed. This points to the general problem that the specific number of generations~(\Sexpr{ngenerations}) was not actually motivated. %Even a single-generation jump from from 0/50 to 50/50 has a non-zero~(if extremely small) probability and would arguably not be s-shaped.
...
%To be able to compare different strategies of sampling data from the posterior, using the same~(regularising) parameter settings as above.

<<naiveconditioningprobabilities, fig.cap=paste("Probability of having completed at least one transition over time, for ", twotypes[2])>>=
...
For the \emph{averaging} learner with parameters $\frac{\alpha}{2}=\Sexpr{alphahalved}$ and $N=\Sexpr{N}$ the chain is most likely to first reach categorical usage of the incoming variant at the distribution's mode after \Sexpr{which.max(data)} generations, while on average the first transition takes \Sexpr{round(averageduration)} iterations to complete. Knowing this we can now generate some of these more `typical' trajectories.

During the random generation we will even go one step further by limiting ourselves to only those transitions that first complete after \emph{exactly} the specific number of generations, i.e.~we exclude ones that reach a frequency of~\Sexpr{N} of the incoming variant early and stay there. Three such example transitions can be seen in Figure~\ref{fig:complexconditioningtransitions}.
...
<<doubleconditioned, fig.cap="Average trajectory of all transitions with the exact same duration. \\textit{(i)}~duration equal to the \\emph{most likely} duration of a transition \\textit{(ii)}~duration equal to the average duration of all completed transitions.", fig.subcap=twotypes>>=
stats <- completionstats(sampler, exactduration=TRUE)

par(mfrow=c(1, 2), pty="s")
plotposterior(exactconditioning(sampler, stats[1]), main="(i)")
plotposterior(exactconditioning(sampler, stats[2]), main="(ii)")


stats <- completionstats(averager, exactduration=TRUE)

par(mfrow=c(1, 2), pty="s")
plotposterior(exactconditioning(averager, stats[1]), main="(i)")
plotposterior(exactconditioning(averager, stats[2]), main="(ii)")
@
...
\subsection{Effect of population size on average duration of transitions}
