...
Here, I will consider three different ways for an individual to generate their own productions~$x'$ based on the learning sample~$x$ that they themselves received. % deriving a specific value~$\theta$ from the posterior distribution,
...
\end{equation}
While \citet{Reali2009} call this a `MAP' learner, I will refer to this mechanism of selecting a hypothesis as the \emph{averager} strategy.
...
<<naiveconditioning, fig.scap="State probability distribution for all Markov chains where the input to the first generation consists of tokens of only one variant", fig.cap=paste("State probability distribution for all Markov chains of length ", ngenerations, " where the input to the first generation consists of tokens of only one variant. The dashed white line indicates the trajectory through the `average' states that the chain is in at any given point in time. \\emph{(i)}~conditioning on the first generation's input only \\emph{(ii)}~conditioning on both the first and final generations' data.", sep=""), fig.subcap=paste("Results with ", twotypes[2], " with $\\alpha=", alphas[-2], "$.", sep="")>>=
...
<<naiveconditioningtransitions, fig.scap=paste("Three randomly generated Markov chains initiated at $0/N$ and terminating at $N/N$ after", ngenerations, "iterations."), fig.cap=paste("Three randomly generated Markov chains initiated at $0/N$ and terminating at $N/N$ after", ngenerations, "iterations.", twotypesstring)>>=
