...
This assumption allows us to easily determine some of the dynamics of the system. For example, assuming that our system would run indefinitely, we can calculate the probability of the system to be in a particular state at any given point in time. The so-called \emph{stationary distribution} of the different systems whose input/mean-output mapping we visualised previously in Figure~\ref{fig:meanmapping} are shown in Figure~\ref{fig:stationarydistribution}.
...
From these stationary distributions we can confirm that the regularisation parameter $\alpha$ works as intended: TODO
...
for (alpha in alphas[-2]) {
  ...
  plotposterior(noconditioning(bilm.transition.matrix.average(N, alpha), ngenerations), addmostlikely=FALSE, main="(i)")
  plotposterior(naiveconditioning(bilm.transition.matrix.average(N, alpha), ngenerations), addmostlikely=FALSE, main="(ii)")
}
@
...
<<naiveconditioningprobabilities, fig.scap="Probability of having completed at least one transition", fig.cap=paste("Probability of having completed at least one transition from categorical usage of one to categorical usage of the other variant, for", twotypes[2], ".", sep="")>>=
...
tightmargin(mfrow=c(1, 2), pty="s")
plotposterior(exactconditioning(sampler, samplerstats[1]), main="(i)")
plotposterior(exactconditioning(sampler, samplerstats[2]), main="(ii)")

tightmargin(mfrow=c(1, 2), pty="s")
plotposterior(exactconditioning(averager, averagerstats[1]), main="(i)")
plotposterior(exactconditioning(averager, averagerstats[2]), main="(ii)")
@
...
The sequence of state transitions with the highest overall likelihood is one that rapidly crosses the mixed-usage area in 10-15~generations, and remains hovering at near-categorical usage of either variant for the rest of the time. (It should be noted that the exact timing of this most likely transition along the time axis is irrelevant, in fact all transitions \emph{parallel} to the one indicated by the dots, i.e.~ones with the same shape but actuating at earlier or later generations, have the exact same probability of occurring.)
...
\subsection{Effect of sample size parameter~$N$ on average duration of transitions}
...
Taken the mapping of these two models literally would mean that the parameter~$N$ in the present model corresponded to the population size of a group of Bayesian learners, each of which uses either of the variants categorically, with the probability of adopting either variant given by~$\hat{\theta}$. % TODO unpack
